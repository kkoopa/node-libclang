...
var Range = function (instance) {
  ...
  this.tokenize = function (tu) {
    ...
    var fredda = ref.readPointer(tokens, 0, tokens.length);
    var otherbuf = new Buffer(8);
    console.log(ref.getType(tokens));
    console.log(count.deref());
    console.log(fredda);
    otherbuf.writeInt32LE(fredda.readInt32LE(0) + 24, 0);
    otherbuf.writeInt32LE(fredda.readInt32LE(4), 4);
    console.log(otherbuf);
    throw 'err';
    //return new TokenList(tu, tokens.deref(), count.deref());
  };
};
